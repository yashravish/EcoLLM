...
// No ML model is used — keyword matching + length heuristic keeps
// classification overhead under 0.5ms per request.
...
// hardKeywords map to their complexity score contribution.
var hardKeywords = map[string]int{
	...
	"optimize":      2,
	"refactor":      2,
	// "step by step" is intentionally absent: multi-step detection handles it below.
}
...
// simpleKeywords contribute a signal without raising complexity.
...
func (c *Classifier) Classify(prompt string) ClassificationResult {
	...
	// Simple keyword signals (don't raise complexity, but inform task type)
	...
	// Multi-step detection raises complexity
